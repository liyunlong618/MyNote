# **【使用Markdown】**
____________________________________
<img src=".\\【使用Markdown】image\\1.png" width="30%">

[**【使用Markdown】**](https://blog.csdn.net/qq_40818172/article/details/126260661)

____________________________________
# **个人常用快捷输入**
____________________________________

**笔记需要的格式-复制粘贴：**
## **<font color=orange>目录</font>**

1. [**笔记开头总目录的链接**](#笔记开头总目录的链接)
2. [**<font color=red> 处理关键点: </font>**](#处理关键点)
3. [**<font color=BlueViolet> 视频链接 </font>**](#视频链接)
4. [**<font color=Crimson> 分隔线 </font>**](#分隔线)
5. [**<font color=DarkSalmon> 目录 </font>**](#目录)
6. [**<font color=DodgerBlue> 蓝图类的类名BP </font>**](#蓝图类的类名BP)
7. [**<font color=DeepPink> 加入图片 60%大小 </font>**](#加入图片-60大小)
8. [**<font color=yellow> cpp类名 </font>**](#cpp中的文件名)
9. [**<font color=Cyan> 空格隔一行 </font>**](#空格隔一行)
10. [**<font color=BlueViolet> 无序副标题 </font>**](#无序副标题)
11. [**<font color=BlueViolet> 有序副标题 </font>**](#有序副标题)
12. [**<font color=Green> 笔记结尾-返回最上面 </font>**](#笔记结尾-返回最上面)
12. [**<font color=Magenta> 头文件/源文件 代码 引用 </font>**](#头文件源文件-代码-引用)
____________________________________
 ## 笔记开头总目录的链接
 ```markdown
 ___________________________________________________________________________________________
### [!!!!!!!!!!!项目根目录!!!!!!!!!!!](./!!!!!!!!!!!项目目录!!!!!!!!!!!.md)
___________________________________________________________________________________________
 ```
>[返回目录](#font-colororange目录font)
____________________________________

### 处理关键点

## <font color=red>处理关键点：</font>
 ```markdown
 # <font color=red>处理关键点：</font>
 ```
>[返回目录](#font-colororange目录font)

<font face="黑体" color=red size=3>1. 这里是关键点</font>
 ```markdown
 <font face="黑体" color=red size=3> 1. 这里是关键点 </font>
 ```

 ```markdown
 <font face="黑体" color=red size=3> 2. 这里是关键点 </font>
 ```

 ```markdown
 <font face="黑体" color=red size=3> 3. 这里是关键点 </font>
 ```

 ```markdown
 <font face="黑体" color=red size=3> 4. 这里是关键点 </font>
 ```

 ```markdown
 <font face="黑体" color=red size=3> 5. 这里是关键点 </font>
 ```

 ```markdown
 <font face="黑体" color=red size=3> 6. 这里是关键点 </font>
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 视频链接
 ```markdown
 [链接备注](网址放这里)
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 分隔线
 ```markdown
___________________________________________________________________________________________
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 目录
 ```markdown
 # <font color=orange> 目录 </font>

1. [标题名](#标题名)
2. [标题名](#标题名)
3. [标题名](#标题名)
4. [标题名](#标题名)
5. [标题名](#标题名)
6. [标题名](#标题名)
7. [标题名](#标题名)
8. [标题名](#标题名)
9. [标题名](#标题名)
10. [标题名](#标题名)
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 蓝图类的类名BP


**_<font color=DodgerBlue> 蓝图类的类名BP </font>_**
 ```markdown
 **_<font color=DodgerBlue> 蓝图类的类名BP </font>_**
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 加入图片 60%大小
 ```markdown
 <img src=".\\配图\\GAS-2.0\\图名.png" width="60%">
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### cpp中的文件名
 **<font color=yellow> cpp类名 </font>**
 ```markdown
  **<font color=yellow> cpp类名 </font>**
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 空格隔一行
 ```markdown
 
&emsp;

 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 无序副标题
```markdown
+ 这里是无序副标题
```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 有序副标题
 ```markdown
 1. 有序副标题
 ```
 ```markdown
 2. 有序副标题
 ```
 ```markdown
 3. 有序副标题
 ```
 ```markdown
 4. 有序副标题
 ```
 ```markdown
 5. 有序副标题
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________
 ### 笔记结尾-返回最上面
 ```markdown
 
___________________________________________________________________________________________

[返回最上面](#font-colororange-目录-font)
___________________________________________________________________________________________
 ```
>[返回目录](#font-colororange目录font)
__________________________________________________________________________

### 头文件/源文件 代码 引用

复制下面的:

````

&emsp;

+ `头文件`中：
```cpp
这里是头文件代码这里是头文件代码这里是头文件代码这里是头文件代码这里是头文件代码这里是头文件代码
```

&emsp;

+ `源文件`中：
```cpp
这里是源文件代码这里是源文件代码这里是源文件代码这里是源文件代码这里是源文件代码这里是源文件代码
```

&emsp;

&emsp;

````
>[返回目录](#font-colororange目录font)







































